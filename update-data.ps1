[CmdletBinding()]
param (
    [switch]$UseCachedRegions,
    [switch]$UseCachedVmSkuNames
)
...
if ($UseCachedRegions) {
    $regions = Get-Content $regionsPath -Encoding utf8
}
else {
    ...
    $regions = az account list-locations `
        ...
        # Stage
        Where-Object { !$_.EndsWith("stg") } | `
        Sort-Object
    $regions | Out-File $regionsPath -Encoding utf8
}

if ($UseCachedVmSkuNames) {
    $vmSkuNames = Get-Content $vmSkusPath -Encoding utf8
}
else {
    $skuNameToInfo = @{}
    $skuNameToRegions = @{}
